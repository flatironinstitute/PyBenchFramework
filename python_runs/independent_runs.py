...
def serverless_fio(args, PyBench_root_dir):
    ...
    for node_iter in nodes:
        if my_line_num <= node_iter:
            for job_count in proc:
                ...
                fio_ob_dict[f"{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['io_type']}"] = handler_class.FIOTool()
                
                fio_ob_dict[f"{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['io_type']}"].setup_command(config_file=f"{PyBench_root_dir}/examples/test_files/{job_number}_{hostname}_{job_count}p_{file_count}f_{args['block_size']}_{args['io_type']}.fio", output_format="json", output_file=f"{log_dir}/{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['block_size']}.json")

                with open(f"{command_log_dir}/{job_number}_{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['platform_type']}_command", 'a') as file:
                    file.write(f"num nodes is 1, job number is {job_count}")
                    tmp_cmd_string = ""
                    for cmd_el in fio_ob_dict[f"{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['io_type']}"].command:
                ...
                print("starting fio?")
                fio_ob_dict[f"{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['io_type']}"].run()
                print("stopping fio?")
    ...
    for node_iter in nodes:
        for job_count in proc:
            file_count = job_count
            json_log_file = f"{log_dir}/{hostname}_{node_iter}_{job_count}p_{file_count}f_{args['block_size']}.json"
            combined_json_log_file = f"{log_dir}/combined_{node_iter}_{job_count}p_{file_count}f_{args['block_size']}.json"
            uncombined_json_log_file = f"{log_dir}/uncombined_{node_iter}_{job_count}p_{args['block_size']}.tmp"
