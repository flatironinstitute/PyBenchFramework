...
def write_job_config_template(PyBench_root_dir, job_number, hostname, local_rank, node_iter, job_count, file_count, block_size, io_type, original_file_contents, args, log_dir, fio_ob_dict, fio_ob_name, difference_tuple):

    file_contents = miscellaneous.reset_file_contents(original_file_contents, args, 1, block_size,log_dir,local_rank, difference_tuple)
    ...
    return fio_job_config

def write_job_command_log_file(PyBench_root_dir, job_number, hostname, local_rank, node_iter, job_count, file_count, block_size, io_type, original_file_contents, args, log_dir, fio_ob_dict, fio_ob_name, command_log_dir):
    with open(f"{command_log_dir}/{job_number}_{hostname}_{local_rank}_{node_iter}_{job_count}p_{file_count}f_{block_size}_{args['platform_type']}_command", 'w') as file:
...
def get_mpi_communicator(my_node_count, node_iter, local_rank, job_count, rank, comm):
    ...
    for node_iter in nodes:
        ...
        for block_size in block_sizes:
            for job_count in proc:
                iteration_retries = 0
                iteration_comm = get_mpi_communicator(my_node_count, node_iter, local_rank, job_count, rank, comm)           
                if iteration_comm != MPI.COMM_NULL:
                    #first index is for difference ratio, the second is for fsync values
                    difference_tuple = (1, -1)
                    ...
                    while difference_tuple[0] >= .05 and iteration_retries <= 3:
                        ...
                        with open(uncombined_json_log_file, 'w') as file:
                            pass
                        ...
                        fio_job_config = write_job_config_template(PyBench_root_dir, job_number, hostname, local_rank, node_iter, job_count, file_count, block_size, args['io_type'], original_file_contents, args, log_dir, fio_ob_dict, fio_ob_name, difference_tuple)
                        ...
                        write_job_command_log_file(PyBench_root_dir, job_number, hostname, local_rank, node_iter, job_count, file_count, block_size, args['io_type'], original_file_contents, args, log_dir, fio_ob_dict, fio_ob_name, command_log_dir)
                        ...
                        if rank == 0:
                            difference_tuple = log_and_analyze_data_points(log_dir, fio_ob_dict[fio_ob_name], start_end_times_list, fio_job_config, difference_tuple)

                        difference_tuple = iteration_comm.bcast(difference_tuple, root=0)
                        ...
                        if os.path.exists(json_log_file):
                            ...
                            with open(uncombined_json_log_file, 'a') as file:
                                ...
                                file.write(f"{hostname}, bw: {bw}, iops: {iops}, rank: {rank}\n")
                                fcntl.flock(file, fcntl.LOCK_UN)  # Unlock the file after writing
                        else:
                            print(f"{datetime.datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] {rank} FIO JSON LOG FILE DOESN'T EXIST!!! - iteration {iteration_count}")
                            sys.exit()
