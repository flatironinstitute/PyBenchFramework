                ...
                if args['timed']:
                    ...
                    if elapsed_time <= lower_time_threshold or elapsed_time >= upper_time_threshold:
                        while elapsed_time <= lower_time_threshold or elapsed_time >= upper_time_threshold:
                            ...
                            mdtest_obj_dict[f"{node_type}_nodes_{ranks_per_node}_ranks_{new_files_per_rank}_new_files_per_rank"].setup_command(config=config, config_file=f"{PyBench_root_dir}/{args['config']}", mpi_ranks=f"{tmp_rank}", files_per_rank=f"{new_files_per_rank}", directory=f"{directory}", output_file=out_file, ranks_per_node=f"{ranks_per_node}", write_output=args['write_output'])
