...
def independent_ranks(args, PyBench_root_dir):
    ...
    resume_log_dir = args.get('incomplete_job')

    if rank == 0:
        node_block_job_combo = miscellaneous.check_previous_job(args, nodes, block_sizes, proc)
    else:
        node_block_job_combo = None

    node_block_job_combo = comm.bcast(node_block_job_combo, root=0)
        ...
        for block_size in block_sizes:
            for job_count in proc:
                ...
                if iteration_comm != MPI.COMM_NULL:
                    ...
                    if node_block_job_combo[f"{node_iter}_{block_size}_{job_count}"] == False:
                        ...
                        if os.path.exists(fio_job_config):
                            os.remove(fio_job_config)
                        
                        # each rank builds its own tuple
                        combined_times = (hostname, rank, start_time, end_time)

                        # allgather returns a list with one item per rank, ordered by rank
                        start_end_times_list = iteration_comm.allgather(combined_times)

                        # everyone can print/log
                        print(starting_statement)
                        print(ending_statement)

                        # everyone computes variability with the same full list
                        variability = log_and_analyze_data_points(
                            log_dir, fio_ob_dict[fio_ob_name], start_end_times_list
                        )

                        temporary_file_contents = original_file_contents
                        optimization_counter = 0
                        while args['optimize_rate'] == "True" and variability and optimization_counter <= 3:
                            print("optimizing iops rate:")
                            # Here we run an optimization loop and overwrite the files for this iteration

                            # __________________________________________________________________
                            #Reset file contents for FIO config file

                            file_contents = miscellaneous.reset_file_contents(temporary_file_contents, args, 1, block_size,log_dir,local_rank, variability)
                            fio_job_config = f"{PyBench_root_dir}/examples/test_files/{job_number}_{hostname}_{local_rank}_{node_iter}n_{job_count}p_{file_count}f_{block_size}_{args['io_type']}.fio"
                            temporary_file_contents = file_contents
                            with open(fio_job_config, 'w') as file:
                                file.write(file_contents)
                        ...
                        print(f"Skipping iteration of node count: {node_iter}, block size: {block_size}, job count: {job_count}...")
