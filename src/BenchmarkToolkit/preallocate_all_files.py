...
for node_count in nodes:
    for job_count in proc:
        ...
        with open(
            f"examples/test_files/multinode_{job_count}p_{file_count}f_{args['block_size']}_{args['io_type']}.fio",
            "w",
        ) as file:
            file.write(file_contents)

        fio_ob_dict[f"{node_count}n_{job_count}p_{file_count}f_{args['io_type']}"] = (
            handler_class.FIOTool()
        )

        fio_ob_dict[
            f"{node_count}n_{job_count}p_{file_count}f_{args['io_type']}"
        ].setup_command(
            config_file=f"examples/test_files/multinode_{job_count}p_{file_count}f_{args['block_size']}_{args['io_type']}.fio",
            output_format="json",
