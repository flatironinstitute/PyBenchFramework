...
class NetworkCounterCollection:
    ...
    def monitor_traffic(
        self, args, proc_number, node_count, block_size, PyBench_root_dir
    ):
        # global stop_thread
        ...
        with open(
            f"{root_dir}/network_stats/{slurm_job_number}/{hostname}_{node_count}_{proc_number}_{block_size}_{args['io_type']}",
            "a",
        ) as file:
            while not self.stop_thread:
                epoch_time = int(time.time())

                bytes_sent, bytes_recv = self.calculate_net_rate(
                    args["interface_name"], hostname
                )
    ...
    def calculate_net_rate(self, interface_name, hostname, interval=1):
        ...
        if interface_name in old_stats:
            ...
            bytes_sent_per_sec = (
                new_interface_stats.bytes_sent - old_interface_stats.bytes_sent
            ) / interval
            bytes_recv_per_sec = (
                new_inteface_stats.bytes_recv - old_inteface_stats.bytes_recv
            ) / interval
