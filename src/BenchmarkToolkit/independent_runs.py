...
def serverless_fio(args):
    ...
    def background_network_monitor(
        args, job_count, node_count, block_size, PyBench_root_dir
    ):
        print("network_monitoring")
        network_counter_collection.monitor_traffic(
            args, job_count, node_count, block_size, PyBench_root_dir
        )
    ...
    for node_iter in nodes:
        ...
        if rank <= node_iter - 1:
            ...
            new_comm = comm.Split(
                color=MPI.UNDEFINED, key=rank
            )  # Exclude ranks <= some_count
            ...
            for block_size in block_sizes:
                ...
                for job_count in proc:
                    ...
                    file_contents = miscellaneous.reset_file_contents(
                        original_file_contents, args, job_count, block_size, log_dir
                    )
                    ...
                    fio_ob_dict[fio_ob_name].setup_command(
                        ...
                        output_format="json",
                        output_file=f"{log_dir}/{hostname}_{node_iter}_{job_count}p_{file_count}f_{block_size}.json",
                    )

                    with open(
                        f"{command_log_dir}/{job_number}_{hostname}_{node_iter}_{job_count}p_{file_count}f_{block_size}_{args['platform_type']}_command",
                        "a",
                    ) as file:
                    ...
                    background_thread = threading.Thread(
                        target=background_network_monitor,
                        args=(args, job_count, node_iter, block_size, PyBench_root_dir),
                    )
                    ...
                    print(
                        f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] starting fio Job num: {job_count}, node count: {node_iter}, IO type {args['io_type']} {time.time()}"
                    )
                    fio_ob_dict[fio_ob_name].run()
                    print(
                        f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] stopping fio Job num: {job_count}, node count: {node_iter}, IO type {args['io_type']} {time.time()}"
                    )
                    ...
                    print(
                        f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Job num: {job_count}, node count: {node_iter}. Iteration is finished. {hostname} [s-{start_time}], [e-{end_time}, el-{elapsed_time}]"
                    )
                    ...
                    first_barrier_file = (
                        f"{log_dir}/barrier_file_1_{iteration_count}.txt"
                    )
                    second_barrier_file = (
                        f"{log_dir}/barrier_file_2_{iteration_count}.txt"
                    )
                    ...
                    if os.path.exists(json_log_file):
                        ...
                        print(
                            f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] FIO JSON LOG FILE DOESN'T EXIST!!! - iteration {iteration_count}"
                        )
                        sys.exit()
    ...
    for node_iter in nodes:
        for block_size in block_sizes:
            for job_count in proc:
                ...
                uncombined_json_log_file = (
                    f"{log_dir}/uncombined_{node_iter}_{job_count}p_{block_size}.tmp"
                )
                if os.path.exists(json_log_file):
                    bw, iops = miscellaneous.load_json_results(json_log_file)
                ...
                if rank == 0:
                    ...
                    with open(combined_json_log_file, "w") as json_file:
                        ...
                        print(
                            f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Data successfully written to {combined_json_log_file}"
                        )
