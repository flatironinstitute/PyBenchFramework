...
def wrap_mdtest(args):
    ...
    files_per_rank_list = sorted(
        benchmark_tools.split_arg_sequence(args["files_per_rank"], "--files-per-rank")
    )
    ...
    if args["timed"]:
        ...
        if len(times) != 2:
            print(
                f"When using the 'timed' option, please ensure to specify two comma-delimited integer values indicating a lower threshold and upper threshold of time in seconds that the test should run for. Values as interpreted are: {times}"
            )
            sys.exit()
    ...
    for node in node_count:
        for rank in mpi_ranks:
            ...
            for files_per_rank in files_per_rank_list:
                ...
                print(
                    f"ranks per node are {ranks_per_node} and type is {type(ranks_per_node)}, nodes are {node_type} and type is {type(node_type)}"
                )
                mdtest_obj_dict[
                    f"{node_type}_nodes_{ranks_per_node}_ranks_{files_per_rank}_files_per_rank"
                ...
                mdtest_obj_dict[
                    ...
                    test_repetition=f"{test_repetition}",
                ...
                with open(
                    f"{command_log_dir}/mdtest_command_{node_type}_nodes_{ranks_per_node}_ranks_{files_per_rank}_files_per_rank",
                    "a",
                ) as file:
                ...
                if args["timed"]:
                    ...
                    if (
                        ...
                        while (
                            ...
                            source = out_file
                            tmp_log_filename = re.split("/", out_file)[
                                len(re.split("/", out_file)) - 1
                            ]
                            ...
                            if elapsed_time <= lower_time_threshold:
                                ...
                                new_files_per_rank = int(
                                    files_per_rank * multiple
                                ) + int(0.05 * files_per_rank)
                            elif elapsed_time >= upper_time_threshold:
                                multiple = upper_time_threshold / elapsed_time
                                new_files_per_rank = int(
                                    files_per_rank * multiple
                                ) - int(0.05 * files_per_rank)
                            else:
                                multiple = new_files_per_rank = 0

                            out_file = f"{log_dir}/mdtest_output_{node_type}_nodes_{ranks_per_node}_ranks_{new_files_per_rank}_new_files_per_rank_timed"

                            mdtest_obj_dict[
                                f"{node_type}_nodes_{ranks_per_node}_ranks_{new_files_per_rank}_new_files_per_rank"
                            ] = handler_class.mdtestTool()
                            mdtest_obj_dict[
                                f"{node_type}_nodes_{ranks_per_node}_ranks_{new_files_per_rank}_new_files_per_rank"
                            ].setup_command(
                                config_file=f"{args['config']}",
                                mpi_ranks=f"{tmp_rank}",
                                files_per_rank=f"{new_files_per_rank}",
                                test_repetition=f"{test_repetition}",
                                directory=f"{directory}",
                                offset=f"{offset}",
                                output_file=out_file,
                            ...
                            with open(
                                f"{command_log_dir}/mdtest_command_{node_type}_nodes_{ranks_per_node}_ranks_{new_files_per_rank}_new_files_per_rank_timed",
                                "a",
                            ) as file:
                                file.write("The following is the mdtest command")
                                tmp_cmd_string = ""
                            ...
                            print(
                                f"entered the optimizer. Old elapsed time: {old_elapsed_time}, New elapsed time: {elapsed_time}, old files_per_rank {files_per_rank}, new files per rank {new_files_per_rank}, multiple is: {multiple}"
                            )
                            files_per_rank = new_files_per_rank
                    total_files_optimized = files_per_rank * tmp_rank
                print(
                    f"mdtest job {node_type}_nodes_{ranks_per_node}_ranks_{files_per_rank}_files_per_rank is finished. [s-{start_time}], [e-{end_time}], elapsed time: {elapsed_time}"
                )
