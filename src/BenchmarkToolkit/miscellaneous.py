...
def reset_file_contents(
    original_file_contents, args, job_count, single_block_size, log_dir, local_rank
):
    ...
    file_contents = file_contents.replace(
        "__hostname__", f"{mapped_hostname}.{local_rank}"
    )
...
def load_ior_json_results(filename, log_dir):
    ...
    filtered_lines = [
        line for line in lines if not any(s in line for s in strings_to_remove)
    ]
    ...
    with open(filename, "r") as file:
        try:
            ...
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} IOR workflow: Error decoding JSON From file: {filename}"
            )
    bw = {}
    iops = {}
    ...
    try:
        ...
        print(
            f"{datetime.now().strftime('%b %d %H:%M:%S')} An unexpected error occurred: {e}. IOR, filename: {filename}"
        )
    ...
    if filename.endswith(".json"):
        with open(filename, "r") as file:
            try:
                ...
                print(
                    f"{hostname} {datetime.now().strftime('%b %d %H:%M:%S')} Error decoding JSON from file: {filename}"
                )
...
def restart_ceph_unit(path):
    def check_ceph_is_active(escaped_path):
        ...
        try:
            ...
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Ceph is active? Output: ",
                result.stdout,
            )
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Ceph is active? Errors: ",
                result.stderr,
            )
            ...
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} Failed to get active status for {unit_filename}: {e.stderr}"
            )
            sys.exit(1)
    ...
    print(
        f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Ceph restart? Output: ",
        result.stdout,
    )
    print(
        f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Ceph restart? Errors: ",
        result.stderr,
    )
    ...
    while active_status == 0:
        ...
        if active_counter == 10:
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] systemd unit for {path} is not becoming active"
            )
            sys.exit(1)
