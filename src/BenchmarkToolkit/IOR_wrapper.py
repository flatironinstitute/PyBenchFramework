...
def wrap_IOR(args):
    ...
    if "output_format" in args.keys():
        ...
        # RB: FIXME: No clue what the default should be here
        output_format = "json"
    ...
    for nodes in node_count:
        for ranks in mpi_ranks:
            print(
                f"BEGINNING OF LOOPS _---------------------- {ranks} and nodes {nodes}"
            )
    ...
    for nodes in node_count:
        for ranks in mpi_ranks:
            ...
            data = {"nodes": nodes, "processors": ranks, "bw": bw, "iops": iops}
