...
def barrier_phase(
    file_path, iteration, hostname, phase, node_count, timeout=600, check_interval=1
):
    """
    Phase barrier:
    ...
    while True:
        ...
        if count == node_count:
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Phase={phase}, iteration={iteration} has {count}/{node_count} lines. Barrier passed."
            )
            return
        else:
            print(
                f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Phase={phase}, iteration={iteration} sees only {count}/{node_count} lines. Waiting..."
            )
            time.sleep(check_interval)
            waited += check_interval
            if waited >= timeout:
                print(
                    f"{datetime.now().strftime('%b %d %H:%M:%S')} [{hostname}] Timed out waiting for barrier, Phase={phase}, iteration={iteration}. Exiting."
                )
                sys.exit(1)
