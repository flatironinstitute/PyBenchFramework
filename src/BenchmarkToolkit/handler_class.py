...
class BenchmarkTool(ABC):
    ...
    def setup_command(self, **params):
        ...
        self.params = (
            params  # Ensure params are stored whenever setup_command is called
        )
    ...
    def set_noscrub(self):
        ...
        initial_output = execute_ssh_command(
            "cephmon900", "ceph", "ceph status | grep flags"
        )
        ...
        if initial_output:
            ...
            if noScrub == 0 or noDeepScrub == 0:
                print(
                    f"one of the flags was unset (deep = {noDeepScrub}, scrub = {noScrub}), ensuring both are set..."
                )
                ...
                print(
                    execute_ssh_command(
                        "cephmon900", "ceph", "ceph osd set nodeep-scrub"
                    )
                )
            else:
            ...
            print(execute_ssh_command("cephmon900", "ceph", "ceph osd set noscrub"))
            print(
                execute_ssh_command("cephmon900", "ceph", "ceph osd set nodeep-scrub")
            )
            ...
            if "write_output" in self.params and self.params["write_output"] == 1:
                with open(self.params["output_file"], "a") as output_file:
                    ...
                    if not process.stdout:
                        raise ValueError(
                            "Process output stream is empty. Check the command or its parameters."
                        )
                ...
                start_time = time.time()
                _ = subprocess.run(
                    self.command, capture_output=False, text=True, check=True, env=env
                )
                end_time = time.time()
                elapsed_time = end_time - start_time
...
class mdtestTool(BenchmarkTool):
    def setup_command(self, **params):
        ...
        if files_per_rank:
            ...
            raise ValueError(
                "Number of files per rank must be specified (--files-per-rank)"
            )
    ...
    def run(self):
        ...
        try:
            result = subprocess.run(
                submit_command, text=True, capture_output=True, check=True
            )
