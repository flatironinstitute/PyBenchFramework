#!/bin/bash -e
#SBATCH -o logs/client_modules-ssd-kernel-ec63-%j.log
#SBATCH -e logs/client_modules-ssd-kernel-ec63-%j.err
#SBATCH -p scc
#SBATCH --nodes=20
...
srun --nodes=20 python python_runs/run.py --benchmark "fio-serverless" --slurm-job-number ${SLURM_JOB_ID} --io-type "randwrite" --config python_runs/config/nvme_test_kernel_ssd_ec63.yml --first-node ${first_node} --total-node-count 20
