#!/bin/bash -e
#SBATCH -o logs/client_modules-ssd-fuse-ec63-%j.log
#SBATCH -e logs/client_modules-ssd-fuse-ec63-%j.err
#SBATCH -p scc
#SBATCH --nodes=20
...
srun --nodes=20 python python_runs/run.py --benchmark "fio-serverless" --slurm-job-number ${SLURM_JOB_ID} --block-size 4M --config python_runs/config/nvme_test_fuse_ssd_ec63.yml --first-node ${first_node} --total-node-count 20 --node-count 20 --template-path /mnt/home/skrit/Documents/testing_clones/clone1/PyBenchFramework/examples/template/starting_template.fio --job-number 48 

srun --nodes=20 python python_runs/run.py --benchmark "fio-serverless" --slurm-job-number ${SLURM_JOB_ID} --io-type "read" --config python_runs/config/nvme_test_fuse_ssd_ec63.yml --first-node ${first_node} --total-node-count 20

#srun --nodes=20 python python_runs/run.py --benchmark "fio-serverless" --slurm-job-number ${SLURM_JOB_ID} --io-type "write" --config python_runs/config/nvme_test_fuse_ssd_ec63.yml --first-node ${first_node} --total-node-count 20

srun --nodes=20 python python_runs/run.py --benchmark "fio-serverless" --slurm-job-number ${SLURM_JOB_ID} --io-type "randread" --config python_runs/config/nvme_test_fuse_ssd_ec63.yml --first-node ${first_node} --total-node-count 20

srun --nodes=20 python python_runs/run.py --benchmark "fio-serverless" --slurm-job-number ${SLURM_JOB_ID} --io-type "randwrite" --config python_runs/config/nvme_test_fuse_ssd_ec63.yml --first-node ${first_node} --total-node-count 20
